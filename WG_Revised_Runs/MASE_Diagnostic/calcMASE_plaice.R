...
########## Round 4 ########## 
# calculates MASE for 29F-5
##### Read in all fitted models - round 4 #####
modelRuns4 <- paste(here::here(), "WG_Revised_Runs",
                    c("WHAM_Run29F-4_splitNEFSC-BigUnits-nlAgeComp-fix1/WHAM_Run29F4_model.rds",
                      "WHAM_Run29F-5_splitNEFSC-BigUnits-nlAgeComp-11Match/WHAM_Run29F-5_model.rds"), sep="/")
models <- lapply(modelRuns4, readRDS)
names(models) <- paste("Run", c("29F4", "29F5"))

##### Calculate hindcast - round 4 #####
# Set up storage
df.colnames <- c("hindcast","horizon","mase","model")
df <- as.data.frame(matrix(NA, ncol = length(df.colnames), nrow = 0))
colnames(df) <- df.colnames

# Loop over models - here only second model 29F-5 since 29F-4 already calculated
for(imodel in 1:length(models)){
  n.i <- models[[imodel]]$env$data$n_indices # Number of indices
  models[[imodel]] <- hindcast(models[[imodel]], horizon=1:5, drop=list(indices=1:n.i, index_paa=1:n.i)) # drop both agg and paa for indices 1:#indices
  tmp = models[[imodel]]$hindcast$mase
...
##### Save MASE results - round 4 #####
res_dir4 <- paste(here::here(), "WG_Revised_Runs", "MASE_Diagnostic", sep="/") # Specify storage
saveRDS(models, file=file.path(res_dir4, "models4.rds")) # Save one data object with all models for which MASE was calculated
saveRDS(df, file=file.path(res_dir4, "mase4.rds")) # Save MASE results for these models
...
round2 <- readRDS(file.path(res_dir, "mase2.rds"))
renameHindcast2 <- factor(c(rep("AlbSpring",5),
                            rep("BigSpring",5),
                            rep("AlbFall",5),
                            rep("BigFall",5),
...
round3 <- readRDS(file.path(res_dir, "mase3.rds"))
renameHindcast3 <- factor(c(rep("AlbSpring", 5),
                            rep("BigSpring", 5),
                            rep("AlbFall", 5),
                            rep("BigFall",5),
...
round4 <- readRDS(file.path(res_dir, "mase4.rds"))
### Post-processing
# Since the index numbering varies by run the below provides run-specific labels, all MASE calculations dropped the aggregate index and paa for the same index at the same time
...
##### Plot MASE results #####
### Start plots
png(file.path(res_dir, "mase_model_horizon_5yr_v3.png"), units='in', res=300, width=6, height=4.5)
ggplot(df, aes(x=horizon, y=mase, fill=model, group=interaction(model,horizon))) +
  geom_boxplot(position = position_dodge(width=.75)) +
  geom_hline(yintercept=1, linetype=2) +
  ylim(0,2.5) +
...
png(file.path(res_dir, "mase_model_horizon_3yr_v3.png"), units='in', res=300, width=6, height=4.5)
ggplot(df3, aes(x=horizon, y=mase, fill=model, group=interaction(model,horizon))) +
  geom_boxplot(position = position_dodge(width=.75)) +
  geom_hline(yintercept=1, linetype=2) +
  ylim(0,2.5) +
...
png(file.path(res_dir, "mase_model_5yr_v3.png"), units='in', res=300, width=5, height=5)
ggplot(df, aes(x=model, y=mase, fill=model)) +
  geom_boxplot() +
  geom_hline(yintercept=1, linetype=2) +
  ylim(0,2.5) +
...
png(file.path(res_dir, "mase_model_3yr_v3.png"), units='in', res=300, width=5, height=5)
ggplot(df3, aes(x=model, y=mase, fill=model)) +
  geom_boxplot() +
  geom_hline(yintercept=1, linetype=2) +
  ylim(0,2.5) +
...
png(file.path(res_dir, "mase_index_horizon_5yr_v3.png"), units='in', res=300, width=11.1, height=3.76)
ggplot(df, aes(x=horizon, y=mase, fill=model, group=interaction(model,horizon))) +
  geom_hline(yintercept=1, linetype=2) +
  geom_point(shape=21, size=2.5, position = position_dodge(width=.75)) +
  facet_wrap(~hindcast, nrow=1) +
...
png(file.path(res_dir, "mase_index_horizon_3yr_v3.png"), units='in', res=300, width=11.1, height=3.76)
ggplot(df3, aes(x=horizon, y=mase, fill=model, group=interaction(model,horizon))) +
  geom_hline(yintercept=1, linetype=2) +
  geom_point(shape=21, size=2.5, position = position_dodge(width=.75)) +
  facet_wrap(~hindcast, nrow=1) +
...
png(file.path(res_dir, "mase_index_5yr_v3.png"), units='in', res=300, width=4, height=4)
df %>% filter(mase != Inf) %>% # Filter out indices that don't have data at the end of the time series (Albatross indices)
  ggplot(., aes(x=hindcast, y=mase)) +
  geom_boxplot(fill='grey',alpha=0.5) +
  geom_hline(yintercept=1, linetype=2) +
...
png(file.path(res_dir, "mase_index_3yr_v3.png"), units='in', res=300, width=4, height=4)
df3 %>% filter(mase != Inf) %>% # Filter out indices that don't have data at the end of the time series (Albatross indices)
  ggplot(., aes(x=hindcast, y=mase)) +
  geom_boxplot(fill='grey',alpha=0.5) +
  geom_hline(yintercept=1, linetype=2) +

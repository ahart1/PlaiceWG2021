...
modelsetup <- c(rep("logistic", asap3$dat$n_fleet_sel_blocks), rep("age-specific", use_n_indices))# rep("age-specific", use_n_indices))

init_fleet_sel <- list(c(2,0.4)) # logistic parameters, based on model type
init_index_sel <- lapply(1:use_n_indices, function(x) rep(0.5, input$data$n_ages)) # all VAST spring
...
input <- prepare_wham_input(asap3, selectivity = sel_list, NAA_re = NAA_re) 
temp <- fit_wham(input, MakeADFun.silent = TRUE, do.osa=FALSE, do.sdrep = F, do.retro = F) 
temp$rep$selAA[[2]][1,]
temp$rep$selAA[[3]][1,]

fix_index_sel[[1]] <- c(9) # Fix age 4 for for index 1 all VAST spring
fix_index_sel[[2]] <- c(10) # Fix age 5 for for index 2 all VAST fall
init_index_sel[[1]][9] = 1
init_index_sel[[2]][10] = 1

sel_list <- list(model = modelsetup, # list selectivity model for each fleet and index
                 initial_pars = c(init_fleet_sel, init_index_sel),
                 fix_pars = c(fix_fleet_sel, fix_index_sel))
input <- prepare_wham_input(asap3, selectivity = sel_list, NAA_re = NAA_re) 
temp <- fit_wham(input, MakeADFun.silent = F, do.osa=FALSE, do.sdrep = T, do.retro = F) 
temp$rep$selAA[[2]][1,]
temp$rep$selAA[[3]][1,]
#TJM STOPPED HERE.
